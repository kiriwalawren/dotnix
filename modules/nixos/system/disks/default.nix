{
  ...
  diskGroupModule = {name, ...}: {
    options = {
      ...
      raidLevel = lib.mkOption {
        type = lib.types.nullOr (lib.types.enum [0 1 5 6 10]);
        default = null;
  ...
  mkAssertions = mountPoint: group: let
    deviceCount = builtins.length group.devices;
    minDevices =
      if group.raidLevel != null
      then minDevicesForRaid."${toString group.raidLevel}"
      else 1;
  in [
    {
