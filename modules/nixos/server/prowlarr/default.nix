...
in {
  ...
  config = {
    ...
    systemd.services."prowlarr-indexers" = optionalAttrs (config.server.prowlarr.config.indexers != []) (mkProwlarrIndexersService "prowlarr" config.server.prowlarr.config);
  };
}
